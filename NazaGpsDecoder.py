...
class NazaGpsDecoder:
    ...
    def readRawMessage(self):
        ...
        for i in rcv:
            ...
            payload.append(tmp)
            self._updateCs(tmp)
        mc1 = ord(self.port.read(1));
        mc2 = ord(self.port.read(1))
    ...
    def decodeVerMessage(self, message):
        decoded = {}
        decoded["fw"] = "v%d.%d.%d.%d" % (
        message["payload"][7], message["payload"][6], message["payload"][5], message["payload"][4])
        decoded["hw"] = "v%d.%d.%d.%d" % (
        message["payload"][11], message["payload"][10], message["payload"][9], message["payload"][8])
        return decoded
    ...
    def decodeComMessage(self, message):
        ...
        if (y < self.magYMin):
            self.magYMin = y
        headingNc = (-1 * math.atan2(y - ((self.magYMax + self.magYMin) / 2),
                                     x - ((self.magXMax + self.magXMin) / 2)) * 180.0) / math.pi;
        if (headingNc < 0):
            headingNc += 360.0;
...
if __name__ == '__main__':
    ...
    decoder = NazaGpsDecoder(device = 'com12')
    while(True):
        a = decoder.readRawMessage()
        #print(decoder.decodeMessage(a))
        print(decoder.readMessage("gps")['decoded']['lon'],decoder.readMessage("gps")['decoded']['lat'],decoder.readMessage("gps")['decoded']['satelites'],decoder.readMessage("gps")['decoded']['fix'])
        # print
        # "SELFTEST", a
        # print
        # "SELFTEST", decoder.decodeMessage(a)
        # print
        # "SELFTEST", decoder.readMessage("ver")
        # print
        # "SELFTEST", decoder.readMessage("com")
        # print
        # "SELFTEST", decoder.readMessage("gps")